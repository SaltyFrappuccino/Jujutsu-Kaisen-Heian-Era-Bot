...
MAX_NAV_BUTTONS_IN_ROW = 2 

def get_main_menu_keyboard(is_admin=False):
    keyboard = VkKeyboard(one_time=False)
    keyboard.add_button("Просмотреть всю Анкету", color=VkKeyboardColor.PRIMARY)
    keyboard.add_line()
    keyboard.add_button("Просмотреть основную информацию", color=VkKeyboardColor.SECONDARY)
    keyboard.add_button("Просмотреть характеристики", color=VkKeyboardColor.SECONDARY) 
    keyboard.add_line()
    keyboard.add_button("Просмотреть Проклятую Технику", color=VkKeyboardColor.SECONDARY) 
    keyboard.add_button("Просмотреть Снаряжение", color=VkKeyboardColor.SECONDARY) 
    keyboard.add_line()
    keyboard.add_button("Изученные Общие Техники", color=VkKeyboardColor.SECONDARY)
    keyboard.add_button("Потратить ОР", color=VkKeyboardColor.POSITIVE)
    keyboard.add_line()
...
def get_admin_panel_keyboard():
    ...
    keyboard.add_button("Редактировать Персонажа", color=VkKeyboardColor.PRIMARY)
    keyboard.add_line()
    keyboard.add_button("Просмотреть Анкеты", color=VkKeyboardColor.PRIMARY) 
    keyboard.add_line()
...
def get_text_input_done_keyboard():
    keyboard = VkKeyboard(one_time=True)
    keyboard.add_button("Готово (сохранить текст)", color=VkKeyboardColor.POSITIVE)
    keyboard.add_button("Отмена", color=VkKeyboardColor.NEGATIVE) 
    return keyboard.get_keyboard()
...
def get_user_character_selection_keyboard(characters_list, action_to_perform):
    ...
    if not characters_list: 
        keyboard.add_button("У вас нет персонажей", color=VkKeyboardColor.NEGATIVE)
    else:
        for i, char_data in enumerate(characters_list): 
            if i >= 9: 
                keyboard.add_button("Больше персонажей...", VkKeyboardColor.DEFAULT); keyboard.add_line(); break
            label = char_data['full_name']
            if len(label) > 35: label = label[:32] + "..."
            keyboard.add_button(label, color=VkKeyboardColor.PRIMARY, 
                                payload={"action": "user_select_char", "char_id": char_data['id'], "next_action": action_to_perform})
            keyboard.add_line()
    keyboard.add_button("Отмена", color=VkKeyboardColor.SECONDARY) 
    return keyboard.get_keyboard()

def get_admin_character_list_keyboard(characters_list, for_action="view", current_page=0, items_per_page=9):
    keyboard = VkKeyboard(one_time=True)
    payload_action = "admin_select_char_view" if for_action == "view" else "select_char_edit"
    ...
    if not paginated_list and current_page == 0 : 
        keyboard.add_button("Нет персонажей", color=VkKeyboardColor.NEGATIVE)
        keyboard.add_line()
    else:
        for char_data in paginated_list:
            ...
            if len(label) > 35: label = label[:32] + "..."
            keyboard.add_button(label, color=VkKeyboardColor.PRIMARY, 
                                payload={"action": payload_action, "char_id": char_data['id']})
            keyboard.add_line()
    ...
    if nav_buttons_row:
        for btn in nav_buttons_row:
            keyboard.add_button(btn["label"], color=VkKeyboardColor.SECONDARY, payload=btn["payload"])
        if len(nav_buttons_row) == 1 and len(paginated_list) > 0:
            keyboard.add_line()
        elif len(nav_buttons_row) == 2:
            keyboard.add_line()
...
def get_stat_selection_keyboard(character_all_data_dict, available_rp, current_page=0): 
    ...
    stat_dependencies = {
        ...
        "level_black_flash_chance": "has_black_flash_learned",
        "level_black_flash_limit": "has_black_flash_learned",
    }
    for stat_key in all_stat_keys_ordered:
        ...
        current_level = character_all_data_dict[stat_key]
        cost = get_stat_upgrade_cost(stat_key, current_level)
    ...
    if not stats_on_this_page and total_eligible_stats == 0 : print("DEBUG KEYBOARDS: No eligible stats to display at all.")
...
def get_admin_edit_stat_fields_keyboard(char_id, character_data_dict, current_page=0): 
    ...
    keyboard = VkKeyboard(one_time=True)
    buttons_added_on_page = 0
    all_stat_keys_ordered = get_all_stat_keys() 
    
    eligible_stats_for_edit = []
    if character_data_dict:
        for stat_key in all_stat_keys_ordered:
            if stat_key in character_data_dict:
                 eligible_stats_for_edit.append(stat_key)
    
    total_stats = len(eligible_stats_for_edit)
    start_index = current_page * ADMIN_STATS_PER_PAGE
    end_index = start_index + ADMIN_STATS_PER_PAGE
    stats_on_this_page_keys = eligible_stats_for_edit[start_index:end_index]

    if character_data_dict is None:
        keyboard.add_button("Ошибка: нет данных персонажа", VkKeyboardColor.NEGATIVE); keyboard.add_line()
    elif not stats_on_this_page_keys and current_page == 0: 
        keyboard.add_button("Нет статов для редактирования", VkKeyboardColor.DEFAULT); keyboard.add_line()
    else:
        for stat_key in stats_on_this_page_keys:
            ...
            keyboard.add_button(label, color=VkKeyboardColor.PRIMARY, payload={"action": "select_edit_stat", "char_id": char_id, "stat": stat_key})
            keyboard.add_line()
            buttons_added_on_page +=1
            
    nav_buttons_row = []
    if current_page > 0:
        nav_buttons_row.append({"label": "<< Назад (статы)", "payload": {"action": "admin_edit_stat_page", "char_id": char_id, "page": current_page - 1}})
    if end_index < total_stats:
        nav_buttons_row.append({"label": "Вперед (статы) >>", "payload": {"action": "admin_edit_stat_page", "char_id": char_id, "page": current_page + 1}})
...
def get_admin_edit_learned_techniques_keyboard(char_id, character_data_dict): 
    ...
    keyboard = VkKeyboard(one_time=True)
    buttons_added = 0
    ordered_tech_keys = get_all_learned_technique_keys() 
    if character_data_dict is None:
        keyboard.add_button("Ошибка: нет данных персонажа", VkKeyboardColor.NEGATIVE); keyboard.add_line()
    else:
        for tech_key in ordered_tech_keys:
            ...
            if len(label)>40: label = label[:37]+"..."
            keyboard.add_button(label, color=VkKeyboardColor.PRIMARY, payload={"action": "toggle_learned_tech", "char_id": char_id, "tech": tech_key})
            keyboard.add_line()
            buttons_added +=1
    keyboard.add_button("Назад (к действиям с персонажем)", color=VkKeyboardColor.SECONDARY, payload={"action": "back_to_edit_actions", "char_id": char_id})
    return keyboard.get_keyboard()
