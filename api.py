...
import sqlite3
import json # Для работы с JSON в user_states.data

# --- Конфигурация Базы Данных ---
DATABASE_URL = "rp_bot.db" # Путь к вашей БД

def get_db_connection():
    conn = sqlite3.connect(DATABASE_URL)
    conn.row_factory = sqlite3.Row # Возвращает строки как словари (или объекты с доступом по ключу)
    return conn

# --- Pydantic Модели для Валидации Данных (Request/Response Bodies) ---
# Модели для таблицы 'characters'
...
class CharacterUpdate(BaseModel): # Для PUT, все поля опциональны
...
class Character(CharacterBase):
    id: int
    class Config:
        orm_mode = True # Для совместимости с объектами БД

# --- Модели для других таблиц (добавьте по аналогии) ---
class UserStateBase(BaseModel):
    vk_id: int
    state: Optional[str] = None
    data: Optional[Dict[str, Any]] = None # Храним как JSON, но Pydantic может работать с dict

class UserStateCreate(UserStateBase):
    pass # vk_id обязателен

class UserState(UserStateBase):
    # vk_id уже есть в Base
    class Config:
        orm_mode = True

# ... и так далее для pending_text, user_nicks, user_stats, user_wallets
...
def create_character_api(character: CharacterCreate, db: sqlite3.Connection = Depends(get_db_connection)):
    ...
    try:
        ...
        # Получаем созданного персонажа для возврата
...
def delete_character_api(character_id: int, db: sqlite3.Connection = Depends(get_db_connection)):
    ...
    try:
        ...
        # Для DELETE успешный ответ обычно 204 No Content, тело ответа не нужно
    except sqlite3.Error as e:
        db.rollback()
...
# --- CRUD операции для 'user_states' (пример) ---
...
def create_or_update_user_state_api(user_state: UserStateCreate, db: sqlite3.Connection = Depends(get_db_connection)):
    # Используем INSERT OR REPLACE для простоты, так как vk_id - PRIMARY KEY
    ...
    try:
        ...
        db.commit()
        # Получаем созданное/обновленное состояние
        ...
        # Преобразуем data из JSON строки обратно в dict для Pydantic модели
...
def delete_user_state_api(vk_id: int, db: sqlite3.Connection = Depends(get_db_connection)):
    ...
    try:
        ...
        if cursor.rowcount == 0:
             # Не ошибка, если состояния и так не было
...
# TODO: Добавьте эндпоинты для остальных таблиц:
# - pending_text (CRUD)
# - user_nicks (CRUD)
# - user_stats (CRUD)
# - user_wallets (CRUD)
